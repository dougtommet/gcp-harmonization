...
uni.item.bank.06 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_06_GCPv2a.rds')) 
uni.item.bank.05 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_05_GCPv2a.rds')) 
``` 
 
### `r gcp_study_nested %>% filter(study_wave_number == 6) %>% pull(study_name_short)`
...
HarmonizationTools::create_summary_report(ggg, uni.item.bank.05, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-06-SAGES_Telephone-uni-mlr-GCPv2a.out'),
   h_obj = ggg,
   itembank = uni.item.bank.05
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.06 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- uni.item.bank.05 %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-06-SAGES_Telephone-uni-mlr-GCPv2a.out')), sep = '
')
```
