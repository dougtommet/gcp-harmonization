...
uni.item.bank.04 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_04_GCPv2a.rds')) 
uni.item.bank.03 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_03_GCPv2a.rds')) 
``` 
 
### `r gcp_study_nested %>% filter(study_wave_number == 4) %>% pull(study_name_short)`
...
ggg <- gcp_study_nested %>% 
  filter(study_wave_number == 4) 
ggg <- ggg[['h_obj']][[1]] 
 
HarmonizationTools::create_summary_report(ggg, uni.item.bank.03, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-04-SAGESII_Telephone-uni-mlr-GCPv2a.out'),
   h_obj = ggg,
   itembank = uni.item.bank.03
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.04 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- uni.item.bank.03 %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-04-SAGESII_Telephone-uni-mlr-GCPv2a.out')), sep = '
')
```
