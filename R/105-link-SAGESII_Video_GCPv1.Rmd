...
uni.item.bank.05 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_05_GCPv1.rds')) 
uni.item.bank.04 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_04_GCPv1.rds')) 
``` 
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-05-SAGESII_Video-uni-mlr-GCPv1.out'),
   h_obj = ggg,
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-05-SAGESII_Video-uni-mlr-GCPv1.out')), sep = '
')
```
