...
uni.item.bank.01 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_01_GCPv1.rds')) 
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link-01-ADAMS-uni-mlr-GCPv1.out'),
   h_obj = ggg,
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link-01-ADAMS-uni-mlr-GCPv1.out')), sep = '
')
```
