...
uni.item.bank.02 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_02_GCPv2c.rds')) 
uni.item.bank.01 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_01_GCPv2c.rds')) 
``` 
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-02-SAGES-uni-mlr-GCPv2c.out'),
   h_obj = ggg,
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-02-SAGES-uni-mlr-GCPv2c.out')), sep = '
')
```
