...
### `r gcp_study_nested %>% filter(study_wave_number == 7) %>% pull(study_name_short)`
...
HarmonizationTools::create_summary_report(ggg, uni.item.bank.06, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-07-INTUIT_ACTIVE-uni-mlr-GCPv2c.out'),
   h_obj = ggg,
   itembank = uni.item.bank.06
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.07 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- uni.item.bank.06 %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-07-INTUIT_ACTIVE-uni-mlr-GCPv2c.out')), sep = '
')
```
