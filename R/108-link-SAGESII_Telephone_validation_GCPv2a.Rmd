...
gcp_study_nested <- readRDS(file=path(r_objects_folder, '070_gcp_study_nested_GCPv2a.rds')) 
uni.item.bank.08 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_08_GCPv2a.rds')) 
uni.item.bank.07 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_07_GCPv2a.rds')) 
``` 
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link-08-SAGESII_Telephone_validation-uni-mlr-GCPv2a.out'),
   h_obj = ggg,
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link-08-SAGESII_Telephone_validation-uni-mlr-GCPv2a.out')), sep = '
')
```
