...
uni.item.bank.02 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_02_GCPv2b.rds')) 
uni.item.bank.01 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_01_GCPv2b.rds')) 
``` 
 
### `r gcp_study_nested %>% filter(study_wave_number == 2) %>% pull(study_name_short)`
...
HarmonizationTools::create_summary_report(ggg, uni.item.bank.01, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link2-02-SAGES-uni-mlr-GCPv2b.out'),
   h_obj = ggg,
   itembank = uni.item.bank.01
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.02 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- uni.item.bank.01 %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link2-02-SAGES-uni-mlr-GCPv2b.out')), sep = '
')
```
