...
uni.item.bank.09 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_09_GCPv2c.rds')) 
uni.item.bank.08 <- readRDS(file=path(r_objects_folder, '090_uni_item_bank_08_GCPv2c.rds')) 
``` 
 
### `r gcp_study_nested %>% filter(study_wave_number == 9) %>% pull(study_name_short)`
...
ggg <- gcp_study_nested %>% 
  filter(study_wave_number == 9) 
ggg <- ggg[['h_obj']][[1]] 
 
HarmonizationTools::create_summary_report(ggg, uni.item.bank.08, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link-09-SAGESII_Video_validation-uni-mlr-GCPv2c.out'),
   h_obj = ggg,
   itembank = uni.item.bank.08
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.09 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- uni.item.bank.08 %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link-09-SAGESII_Video_validation-uni-mlr-GCPv2c.out')), sep = '
')
```
