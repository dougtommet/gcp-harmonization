...
last_itembank <- tibble(filename = dir_ls(here::here("R_objects"), glob = "*090_uni_item_bank_*.rds")) %>%
  separate(filename, into = c(rep(NA, 9), "itembank_filename"), sep = "/") %>%
  filter(str_detect(itembank_filename, "uni_item_bank")) %>%
  filter(str_detect(itembank_filename, gcp_version)) %>%
  slice(n()) %>%
  pull(itembank_filename)



uni_item_bank_last <- readRDS(file=path(r_objects_folder, last_itembank))
gcp_study_nested <- readRDS(file=path(r_objects_folder, str_c("070_gcp_study_nested_", gcp_version, ".rds")))


gcp_long_all_df <- gcp_study_nested %>%
...
my_linking_fx <- function(h_obj, current_item_bank, output_name, 
                          ...
                          a.df_name = "response_df_all_filtered", 
  ...
  HarmonizationTools::link_studies(h_obj = h_obj, 
                                   ...
                                   full_response_df = gcp_long_all_df,
                                   mplus.output.path = here::here("mplus_ignore"), 
...
my_harmonization_fx <- function(i, 
  ...
  if (!swn %in% c(9999)) {
    b.output_name   <- stringr::str_c("link2-", i_pad, "-", study_name, "-uni-mlr-", gcp_version)
...
if (gcp_version == "GCPv1") {
  uni_item_bank_02b <- my_harmonization_fx(2, uni_item_bank_last, 2, b.runmplus = TRUE, b.bifactor_model = FALSE)
} else if (gcp_version == "GCPv2a") {
  uni_item_bank_02b <- my_harmonization_fx(2, uni_item_bank_last, 2, b.runmplus = TRUE, b.bifactor_model = FALSE,
                                          b.extra.model.statement = "[Q226$8 * 7.4];")
...
saveRDS(uni_item_bank_02b,  file=path(r_objects_folder, str_c("091_uni_item_bank_02b_", gcp_version, ".rds")))

# 03 - SAGESII in-person
if (gcp_version == "GCPv1") {
  uni_item_bank_03b <- my_harmonization_fx(3, uni_item_bank_02b, 3, b.runmplus = TRUE, b.bifactor_model = FALSE,
                                           b.extra.model.statement = "[Q104$9 * 7.5];")
} else if (gcp_version == "GCPv2a") {
  uni_item_bank_03b <- my_harmonization_fx(3, uni_item_bank_02b, 3, b.runmplus = TRUE, b.bifactor_model = FALSE,
                                           b.extra.model.statement = "[Q104$9 * 7.5];")
} else {
...
saveRDS(uni_item_bank_05b,  file=path(r_objects_folder, str_c("091_uni_item_bank_05b_", gcp_version, ".rds")))

# 06 - SAGESI telephone
uni_item_bank_06b <- my_harmonization_fx(6, uni_item_bank_05b, 6, b.runmplus = TRUE, b.bifactor_model = FALSE)
saveRDS(uni_item_bank_06b,  file=path(r_objects_folder, str_c("091_uni_item_bank_06b_", gcp_version, ".rds")))

# 07 - Intuit
uni_item_bank_07b <- my_harmonization_fx(7, uni_item_bank_06b, 7, b.runmplus = TRUE, b.bifactor_model = FALSE)
saveRDS(uni_item_bank_07b,  file=path(r_objects_folder, str_c("091_uni_item_bank_07b_", gcp_version, ".rds")))
