...
final_summarize_function <- function(filename, study.nested.named, 
  ...
  cat(str_c("source(here::here('R', '001-Libraries_and_functions.R')) "),                         append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c("source(here::here('R', '005-folder_paths_and_options.R')) "),                        append = TRUE, file = here("R", filename), sep = "\n")
  ...
  cat(str_c(" "),                                                                                                             append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c("HarmonizationTools::create_summary_report(ggg, ", old.itembank.named, ", include.subdomain.reporting = FALSE)"), append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c("```"),                                                                                                           append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c(" "),                                                                                                             append = TRUE, file = here("R", filename), sep = "\n")
  ...
  cat(str_c("#### Mplus output file"),    append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c(" "),                         append = TRUE, file = here("R", filename), sep = "\n")
  
  cat(str_c("```{r  comment=''}"),                                                                               append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c("  cat(readLines(here::here('mplus_output', '", mplus.output, "')), sep = '\n')"),                    append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c("```"),                                                                                  append = TRUE, file = here("R", filename), sep = "\n")
  cat(str_c(" "),                                                                                    append = TRUE, file = here("R", filename), sep = "\n")
}
...
last.mplus.output <- tibble(filename = dir_ls(here::here("mplus_output"), glob = "*link*.out")) %>%
  ...
  filter(str_detect(last_mplus_filename, gcp_version)) %>%
  separate(last_mplus_filename, into = c("link", "study_order", "study_name", "model_type", "estimator", "gcp_version"), sep = "-", remove=FALSE) %>%
  select(-model_type, -estimator, -study_name, -gcp_version) %>%
  mutate(study_order = as.numeric(study_order)) %>%
  arrange(study_order, link) %>%
...
gcp_study_nested %>%
  ...
  mutate(filename = str_c(100+study_order, "-link-", study_name_short2, "_", gcp_version, ".Rmd"),
         ...
         new.itembank.rds = str_c("090_uni_item_bank_", str_pad(study_order, 2, "left", "0"), "_", gcp_version, ".rds"),
         old.itembank.rds = str_c("090_uni_item_bank_", str_pad(study_order-1, 2, "left", "0"), "_", gcp_version, ".rds"),
  ) %>%
  ...
  mutate(goo = pmap(list(filename = filename,
                         ...
                         study.nested.rds = str_c("070_gcp_study_nested_", gcp_version, ".rds"),
