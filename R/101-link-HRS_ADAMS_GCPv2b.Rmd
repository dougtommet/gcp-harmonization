...
empty.item.bank <- readRDS(file=path(r_objects_folder, '070_empty_item_bank.rds')) 
``` 
 
### `r gcp_study_nested %>% filter(study_wave_number == 1) %>% pull(study_name_short)`
...
HarmonizationTools::create_summary_report(ggg, empty.item.bank, include.subdomain.reporting = FALSE)
```
...
item.parameters <- HarmonizationTools::get_item_parameters(summarized_response_df, 
   mplus.output = here::here('mplus_output', 'link-01-ADAMS-uni-mlr-GCPv2b.out'),
   h_obj = ggg,
   itembank = empty.item.bank
) 
...
```{r, results='asis'}
  newbank <- uni.item.bank.01 %>% filter(!str_detect(parameter_threshold, '^s'))
  oldbank <- empty.item.bank %>% filter(!str_detect(parameter_threshold, '^s'))
...
```{r  comment=''}
  cat(readLines(here::here('mplus_output', 'link-01-ADAMS-uni-mlr-GCPv2b.out')), sep = '
')
```
